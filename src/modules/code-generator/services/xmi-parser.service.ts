...
export class XmiParserService {
  ...
  parseXmi(xmlContent: string): DiagramAstData {
    ...
    // D) Reconstruir tablas intermedias / Association Classes (ancla y enlace discontinuo)
    this.resolveAssociationClasses(xmiRoot, rawPackagedElements, nodeMap, connections, connectionSet);
  ...
  private resolveAssociationClasses(
    xmiRoot: any,
    ...
    for (const pair of pairings) {
      ...
      let mainConn: UmlConnectionData | undefined;
      if (pair.mainConnId) {
      ...
      const sourceNode = nodeMap.get(mainConn.sourceNodeId);
      const targetNode = nodeMap.get(mainConn.targetNodeId);
